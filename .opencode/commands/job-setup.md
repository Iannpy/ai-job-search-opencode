---
description: Build your professional profile for the AI Job Search framework
agent: job-assistant
---

# /job-setup - Profile Onboarding

You are running the onboarding setup for the AI Job Search framework. Your goal is to collect the user's professional information and populate all profile files so the `/job-apply` workflow works out of the box.
...
- `outcome.md`: status (in_progress/hired/offer_declined/rejected/no_response/job-interview_only), interview stages, notes. Skip `in_progress` applications for calibration — they have no final signal yet.
...
Documents cover skills, experience, education, references, and behavioral signal. They do not cover everything `/job-apply` and `/job-scrape` need. After the writes, ask follow-up questions for gaps:
...
Then proceed to Step 3 to populate the non-skill files (`AGENTS.md`, `cv/main_example.tex`, `.opencode/skills/job-scraper/search-queries.md`). Step 3 will detect that the seven skill files are already populated and skip those substeps.
...
This section generates the search queries that power `/job-scrape`. Use the information from Sections 1, 4, and 7 to build targeted queries.
...
- **Job portals:** "The framework ships country-agnostic search CLIs (`linkedin-search`, `freehire-search`). `/job-scrape` auto-discovers whatever portal skills are installed under `.agents/skills/`. Which of these fit your market, and do you use other job boards?" WebSearch/`site:` queries remain the fallback for portals without a CLI skill.
...
> - `.opencode/skills/job-scraper/search-queries.md` - Job search queries for `/job-scrape`
>
> **Try it out:**
...
> - Run `/job-setup --section search` later to update your search queries as your priorities evolve
...
- Can be re-run with `--section <name>` to update specific sections (e.g., `/job-setup --section search` to reconfigure job search queries without re-doing the full profile).
